...
- For Steam apps only, the workflow also records `discountEndsAt` (ISO timestamp, or `null`) and `endingSoonNotified` (whether the day-before reminder has already been sent). The end date comes from Steam's `IStoreBrowseService/GetItems` store API (`best_purchase_option.active_discounts[].discount_end_date`); the classic `appdetails` endpoint used for prices does not expose it. If that call fails, end dates are simply omitted and the rest of the run is unaffected. GOG's price API does not expose an end date, so GOG sales carry neither field.
- The same workflow posts a Discord message when a game newly goes on sale (an off-to-on transition versus the previous run, so it never spams while a sale lasts). The message names the game's meeting date, links the store page, and, for Steam, states when the sale ends. It is gated on the repo secret `DISCORD_WEBHOOK_URL`; if the secret is unset, notification is skipped and sale data still updates.
- For Steam sales with a known end date, the workflow also posts a one-time "ending soon" reminder on the daily run within ~30 hours of the end (a window wide enough for the once-a-day cron to reliably catch the final day). `endingSoonNotified` guards against repeats and resets when the game leaves sale or Steam moves the end date. Sale end dates use Discord's `<t:UNIX:R>`/`<t:UNIX:F>` timestamp markup, so each reader sees them in their own locale and timezone.
