...
- Revealed rounds lead with a dedicated winner block, then show the aggregate instant-runoff rounds and the next-round notice. The winner block stacks the full (uncropped) game banner above a copy column holding the game title, the final-round count, genres/playtime, the pitch, the suggester's name when they chose to show it, and the store link. Ranked winners use the final-round count; legacy approval rounds keep their generic aggregate count. Winner artwork uses the same branded no-image fallback as suggestion cards.
...
| `/api/admin/round/:id/announce-winner` | POST | Post the final public Discord winner/meeting announcement after the round is revealed and all announcement fields are ready. Records `winner_announcement_posted` so the button is idempotent. Requires `DISCORD_VOTING_WEBHOOK_URL` in Cloudflare Pages. When `DISCORD_GENERAL_WEBHOOK_URL` is also set, it posts the general-chat results-breakdown link alongside (guarded by `results_link_posted`, shared with the scheduler's reveal path, and best-effort so it never fails the reveal). |
...
| `/api/admin/automation-event` | POST | Record a scheduler automation event (body: `roundId`, `eventType`, optional `payload`). Valid `eventType` values are `suggestions_opened`, `suggestions_halfway_reminded`, `suggestions_last_day_reminded`, `voting_opened`, `voting_halfway_reminded`, `voting_last_day_reminded`, `winner_revealed`, `blocked_alerted`, `winner_setup_needed_alerted`, `winner_announcement_posted`, `results_link_posted`, and `handoff_generated`. Returns `{ ok, duplicate, id }`; a repeat of an already-recorded event returns `{ ok: true, duplicate: true, id: null }` instead of failing. |
...
The `automation_events` table is the idempotency log for the planned voting scheduler. Each row records one automated action taken on a round, keyed by a `UNIQUE (round_id, event_type)` constraint. Known event types are `suggestions_opened`, `suggestions_halfway_reminded`, `suggestions_last_day_reminded`, `voting_opened`, `voting_halfway_reminded`, `voting_last_day_reminded`, `winner_revealed`, `blocked_alerted`, `winner_setup_needed_alerted`, `winner_announcement_posted`, `results_link_posted`, and `handoff_generated`; the database does not enforce a fixed CHECK so new lifecycle events do not require a table rebuild. An optional JSON `payload` captures context such as webhook status, missing fields, and Discord message IDs for rolling public announcements.
...
For votes, submitting a ballot deletes that Discord user's previous rows for the current round and inserts the new ranked choices (one row per ranked game, `rank` 1..N), with the delete and inserts in a single `db.batch()` transaction so a mid-write failure or two overlapping submissions can never lose or double a ballot. That makes the latest ranking count across browsers and devices. Individual ranked ballots are never exposed publicly. The admin Votes section shows the aggregate ranked-choice results first, then lists each individual ballot (expandable, in submission order) so the maintainer can spot-check that the count matches the raw rankings. Ballots are labelled by index, not by who cast them, and Discord ids are never shown.
...
DISCORD_GENERAL_WEBHOOK_URL=...       # optional; admin reveal also posts the results-breakdown link
```
...
The voting scheduler lives under `automation/voting/` as plain ES modules, executed by two independent clocks: the Cloudflare cron Worker in `automation/cron-worker/` (primary, fires punctually) and the GitHub Actions workflow (backstop, see below). All modules except `handoff-node.mjs` and `cli.mjs` are platform-neutral (no `node:` imports) so wrangler can bundle them into the Worker. They are kept side-effect-free where possible so the decision rules stay testable. Each module has a sibling `*.test.mjs` run by `npm test`.
...
- `scheduler.mjs`: pure `decideRoundActions({ today, round, suggestions, rcvResult, automationEvents })`. Given a round's current state it returns one decision:
  ...
  - `open_voting` when the round is `suggesting`, `today` has reached `voting_opens_at` (inclusive), and `voting_opened` is not already recorded. It is also re-emitted as a **recovery** when the round is already `voting` but `voting_opened` was never recorded (e.g. Discord failed between the phase patch and the event record) and voting is genuinely open (`voting_opens_at` reached, close date not passed); the phase re-patch is a harmless no-op and the event stays the lock. A round flipped to `voting` manually ahead of `voting_opens_at` is left alone, since votes are not accepted before that date.
  ...
  - `resume_reveal` (recovery) when the round is `revealed` and `winner_revealed` is recorded but **no** follow-up event exists (`winner_announcement_posted`, `winner_setup_needed_alerted`, or `handoff_generated`), meaning the reveal pass died mid-flight. The runner re-runs the post-reveal side effects (plan, promote when safe, announce or alert, handoff) without re-patching the phase; each step keeps its own idempotency event. A round revealed manually by the admin records no `winner_revealed` event and is deliberately left alone.
...
- `handoff.mjs`: pure winner-promotion planner and maintainer handoff builder.
  ...
  - `buildHandoffMarkdown({ roundPayload, winnerSuggestionId, plan, baseUrl })` renders a maintainer brief: meeting number/title/date, winner details (Steam app id, store URL, GOG when present, banner, genres/platforms, pitch, suggested-by), a "Vote results" section rendered from `rcvResult` (final IRV standing and winner plus a round-by-round summary of counts, eliminations, and exhausted/majority lines, with a legacy approval-tally fallback when `rcvResult` is null and the final-tie blocked state surfaced), `publishReadiness.missing`, explicit HowLongToBeat and localized-description reminders when missing, and a checklist pointing to `MEETING_WORKFLOW.md`.
- `handoff-node.mjs`: Node-only handoff delivery. `handoffArtifactPath(roundId)` and `writeHandoff(markdown, { roundId })` write the brief to `automation-output/meeting-<id>-winner.md` for the GitHub runner to upload as an artifact (never committed; `automation-output/` is gitignored). The cron Worker delivers handoffs its own way instead: `postDiscordFile` in `discord.mjs` posts the Markdown as a file attachment to the private alerts webhook.
- `healthcheck.mjs`: `pingHealthcheck(url)`, the dead-man's-switch ping. Both clocks call it after every successful pass (noop included); it is a no-op without `HEALTHCHECKS_PING_URL` and never throws, so a watchdog hiccup cannot fail a pass. If no ping arrives by the healthchecks.io check's daily deadline, the maintainer is alerted, which turns a silently missed pass (the July 2026 failure mode) into a loud absence.
- `run-scheduler.mjs`: the runner that wires the above together. `runScheduler({ env, today, deps })` reads the environment (`readEnv` requires `VOTING_BASE_URL` and `VOTING_ADMIN_TOKEN`; `DISCORD_VOTING_WEBHOOK_URL` and `DISCORD_VOTING_ALERTS_WEBHOOK_URL` are optional), fetches the current admin round, asks `decideRoundActions` for one decision, and acts on it. Its side-effecting dependencies (`client`, `postDiscord`, `deleteDiscordMessage`, `writeHandoff`, `logger`) are injectable so the flow is unit-tested without real network or filesystem access. `deps.writeHandoff` is required because handoff delivery is host-specific (file for the Node CLI, Discord attachment for the Worker).
- `cli.mjs`: the Node CLI entry used by GitHub Actions (`node automation/voting/cli.mjs`). Owns the Node-specific parts: `process.env`, the filesystem handoff writer from `handoff-node.mjs`, the healthcheck ping, and the non-zero exit code on failure.
...
- `remind_suggestions` / `remind_voting`: post a short general-chat reminder to `DISCORD_GENERAL_WEBHOOK_URL` (the members' general channel, same one as new-suggestion and sale posts) and record its event. Each phase gets two: a halfway reminder (from the window's `midpointDateOnly`, with a games-so-far or ballots-so-far social-proof line) and a last-day reminder (the day before voting opens, and the voting-close date itself since that whole day is votable). Reminders fire only after the phase's opening announcement event exists, so a catch-up pass never posts them out of order; a halfway reminder that was never fired lapses once the last day arrives. Reminders post with `wait: true` and use the same duplicate-cleanup as announcements, but are never deleted otherwise.
...
**Failed posts are never recorded.** When Discord accepts the request the event is recorded; when the webhook is unset the post is skipped and the event is still recorded (the documented degrade-quietly behaviour); but when a real post attempt is rejected (non-ok status) the runner logs a warning and does **not** record the event, so the next pass retries instead of locking the announcement out forever. Combined with the `open_voting`/`resume_reveal` recovery decisions above, a Discord outage at any point in a transition delays a public announcement to the next pass rather than losing it. The `winner_setup_needed_alerted` private alert follows the same post-first, record-on-success ordering as the blocked alert. The one exception is `results_link_posted`, which stays record-first because its event is a lock shared with the admin "Post Discord reveal" path; a lost breakdown link is cosmetic and accepted.

### The two clocks

The daily pass runs from 09:00 Europe/Copenhagen and is driven by two independent clocks. Both are safe to overlap: every action is guarded by the `automation_events` idempotency log, so whichever clock fires first does the work and the other's pass is a no-op.

**Primary: the Cloudflare cron Worker** (`automation/cron-worker/`, deployed as `gamestormers-voting-cron` via `npm run deploy:cron`). Cloudflare cron triggers fire within about a minute of schedule. Triggers at 07:00 and 08:00 UTC cover 09:00 in Denmark across DST, plus a 12:30 UTC same-day backstop; an in-worker gate no-ops any trigger landing before 09:00 local (`shouldRunAt`, computed with `Intl` in `Europe/Copenhagen`). The Worker bundles the same `runScheduler` module and talks to the same admin API over HTTPS, so it holds the same secrets (see [`deployment-guide.md`](deployment-guide.md)). An authenticated `POST /` with Bearer `CRON_TOKEN` runs one pass immediately, bypassing the hour gate like `workflow_dispatch` does; every other request 404s. Handoffs are delivered as a Markdown file attachment on the private alerts webhook instead of a filesystem artifact.

**Backstop: the GitHub Actions workflow** (`.github/workflows/voting-automation.yml`), on `workflow_dispatch` and daily candidate schedules at 07:00, 08:00, and 12:00 UTC using Node 24. A first-step Copenhagen time gate lets any candidate starting at or after 09:00 local proceed and skips earlier ones. GitHub cron triggers routinely start one to four hours late, so the gate is deliberately "at or after 09:00", not "exactly 09"; an exact-hour gate silently skipped the scheduler for days in July 2026, which is why the Worker is now the primary clock. The workflow runs `npm test`, then `node automation/voting/cli.mjs`, then uploads `automation-output/*.md` as the `winner-handoff` artifact (`if-no-files-found: ignore`, so the common no-op run stays green). Permissions are minimal (`contents: read`, `actions: read`); the workflow never commits or edits HTML. It reads the GitHub Actions secrets `VOTING_BASE_URL`, `VOTING_ADMIN_TOKEN`, the optional `DISCORD_VOTING_WEBHOOK_URL` / `DISCORD_VOTING_ALERTS_WEBHOOK_URL`, and the optional `HEALTHCHECKS_PING_URL` (see [`deployment-guide.md`](deployment-guide.md)). Playtime should stay manual unless a reliable source becomes available. (New-suggestion Discord notifications are built separately, see Suggestion Notifications above.)
