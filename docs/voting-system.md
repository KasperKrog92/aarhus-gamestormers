...
| `/api/admin/round/:id` | DELETE | Delete the voting round only. Cascades to its suggestions and votes, but deliberately leaves the public `meetings` row (homepage event/history card) intact. Cancelling or removing a public meeting is a separate, future admin action. |
