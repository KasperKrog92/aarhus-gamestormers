...
`.github/workflows/voting-automation.yml` drives the runner on `workflow_dispatch` and daily from 09:00 Europe/Copenhagen using Node 24. Because GitHub Actions cron is UTC-only, the workflow has candidate schedules for 07:00, 08:00, and 12:00 UTC and a first-step Copenhagen time gate: any candidate that starts at or after 09:00 local proceeds to tests and the scheduler, and earlier candidates skip. GitHub cron triggers routinely start one to four hours late, so the gate is deliberately "at or after 09:00", not "exactly 09"; an exact-hour gate silently skipped the scheduler for days in July 2026. Several same-day candidates may therefore all run the scheduler, which is safe because every action is guarded by the `automation_events` idempotency log, and the 12:00 UTC candidate is a backstop for days where GitHub drops or heavily delays both morning triggers. It runs `npm test`, then `node automation/voting/run-scheduler.mjs`, then uploads `automation-output/*.md` as the `winner-handoff` artifact (`if-no-files-found: ignore`, so the common no-op run stays green). Permissions are minimal (`contents: read`, `actions: read`); the workflow never commits or edits HTML. It reads the GitHub Actions secrets `VOTING_BASE_URL`, `VOTING_ADMIN_TOKEN`, and the optional `DISCORD_VOTING_WEBHOOK_URL` / `DISCORD_VOTING_ALERTS_WEBHOOK_URL` (see [`deployment-guide.md`](deployment-guide.md)). Playtime should stay manual unless a reliable source becomes available. (New-suggestion Discord notifications are built separately, see Suggestion Notifications above.)
