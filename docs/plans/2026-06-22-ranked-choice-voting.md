...
8. `[x]` **Vote-page UI (`js/vote.js`).** Replace the checkbox grid with the click-to-rank + reorder interface; pre-fill from `/api/vote/mine`; submit `{ rankings }`; render `ballotCount` during voting; add/repurpose `STRINGS` (da + en). Update the meeting-flow "Vote" copy.
   - *2026-06-22:* Replaced the approval checkbox with a per-card add/remove toggle plus position badge, and a "Your ranking" panel that reorders via accessible up/down buttons and removes entries (buttons are the baseline; no drag library). Pre-fills from `/api/vote/mine`, submits `{ rankings }`, and relabels the button to "Update ranking" once a ballot exists. Added a turnout line (`round.ballotCount`, singular/plural) shown during voting that re-fetches on submit. Repurposed `introVoting`/`errPickOne`, removed `approve`/`btnVoted`/`alreadyVoted`, added ranking/participation STRINGS in da+en, and updated the meeting-flow Vote copy. New CSS for the ranking controls/panel/turnout; bumped `style.css?v=48` and `vote.js?v=35` on both vote pages. No divergence from the plan.
   - *Verify:* drove the Pages dev server with a member dev-login on a voting-phase round (3 approved games): add/reorder/remove updated badges and numbering, submit posted the ranking and bumped turnout 1→2 (plural), reload pre-filled the saved order with "Update ranking", and the English page mirrored all copy. `npm test` still green (174). Local DB scenario reverted afterward.

9. `[x]` **Reveal UI (`js/vote.js`).** Render the round-by-round explanation from `rcvResult.rounds` under the winner card; keep the legacy fallback for ballot-less rounds.
   - *2026-06-22:* Added `rcvBreakdown(rcvResult, suggestions)` and rendered it under the headline winner cards in `renderRevealed`. Each round shows its number, the majority line (`Majority: {n} of {active}`) and exhausted-ballot count, then every still-standing candidate with vote count, a bar, and a majority-marker tick (bar + marker share the active-ballot denominator). Eliminated candidates are tagged and dimmed, transfers show a `+{n} transferred` chip, and the winning round highlights the winner (green name + Winner tag) with a "Reached a majority and wins." note. Returns null when `rcvResult` is absent, so historical approval rounds and ballot-less rounds keep the legacy sort-by-votes headline grid. Added da+en STRINGS (`breakdownTitle`, `breakdownIntro`, `rcvRound`, `rcvMajority`, `rcvExhaustedOne/Other`, `rcvEliminated`, `rcvWinnerRound`, `rcvTransferred`) and `.vote-breakdown*` CSS. Bumped `style.css?v=49` and `vote.js?v=36` on both vote pages. No divergence from the plan.
   - *Verify:* `npm test` green (174). Drove the Pages dev server on a revealed round (4 candidates, 10 ranked ballots) producing a 3-round IRV count (eliminate Talos, then Call of the Sea; 2 votes transfer to Wanderstop, which reaches majority and wins). The rendered breakdown matched the hand-computed result on both the da and en pages and at mobile width; the ballot-less/legacy fallback is preserved since `rcvBreakdown` returns null without `rcvResult`. Local DB scenario reverted afterward.

10. `[x]` **Admin UI (`vote-admin.html`).** Replace approval counts with first-preference + IRV round-by-round summary (aggregate only); surface the final-tie blocked state; keep ballot count, the privacy note, and ballot deletion.
    - *2026-06-22:* Rewrote the Votes section's `renderBallots(ballots, suggestions, rcvResult)`: a ballot-count line, the existing (reworded) privacy note, an approved-only "First-preference counts" list, and a "Ranked-choice count" card rendered from `rcvResult` (winner line plus per-round majority/active/exhausted meta and per-candidate votes with `+n transferred`, `eliminated`, and `winner, reached majority` tags). When `rcvResult.blocked.reason === 'tie'`, an error banner names the tied games and points the maintainer to the Selected game section. Falls back to the first-preference list only for historical approval rounds (`rcvResult` null). Changed the Votes section-count chip from "N ballots · M approvals" to just the ballot count; dropped the unused `totalApprovals`. No CSS change (used existing admin classes + minimal inline styles), so no `style.css` bump for the admin page.
    - *Verify:* drove the Pages dev server on a revealed round 19 with a hand-built 10-ballot, 4-candidate scenario; the admin breakdown matched the hand-computed 3-round IRV exactly (eliminate FAR, then Talos; Call of the Sea reaches 5 of 9 with 1 exhausted ballot and wins), agreeing with the step-9 public reveal. A separate 2-candidate 1-1 scenario rendered the final-tie banner prompting manual selection. `npm test` green (174). Local DB scenario reverted afterward.

11. `[x]` **Docs + cache busting.** Update `docs/voting-system.md`, `schema.sql` comments, `MEETING_WORKFLOW.md`, and any roadmap/CLAUDE notes; bump `css/style.css` `?v=N` if vote styles changed; update `sitemap.xml` `lastmod` only if public copy changed meaningfully. Land docs in the same commit as the code per repo rules.
    - *2026-06-22:* Rewrote `docs/voting-system.md` for RCV: the `votes` table description, the `/api/round/current` (rcvResult + `ballotCount` turnout) and `/api/vote` rows (now `{ rankings }` with validation), added a `/api/vote/mine` row, updated the `/api/admin/round` row to mention `rcvResult`, reworded the vote-integrity paragraph, added a "Ranked-Choice Counting And Tie Rules" section with algorithm, deterministic elimination tiebreaks, final-tie blocking, and a Privacy subsection (participation-as-turnout and small-electorate caveat), and updated the scheduler module section (`decideRoundActions` now consumes `rcvResult`), the handoff "Vote results" description, and the ranking voting-open copy. Fixed the `schema.sql` header to "ranked-choice voting system" (the `votes` comment block was already updated in step 1). `MEETING_WORKFLOW.md`, `docs/roadmap.md`, and `CLAUDE.md` had no approval-voting/"most votes" wording to change. No `style.css` bump (step 10 added no CSS; vote-page styles were already bumped in steps 8/9). `sitemap.xml` vote-page `lastmod` was already 2026-06-22 from earlier steps, so no change.
